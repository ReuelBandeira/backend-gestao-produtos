...
@injectable()
export default class QuantityCurrentOrderIdlineService {
  constructor(
  ...
  async execute({id_line}: IRequest): Promise<QuantityCurrentOrder> {
    ...
    const filteredData = mergedData.filter(item => item.id_line == id_line);

    return ({line_quantity_current_order:filteredData})
  }
}
