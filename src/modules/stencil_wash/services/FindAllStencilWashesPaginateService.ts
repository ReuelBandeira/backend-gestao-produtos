...
@injectable()
export default class FindAllStencilWashesPaginateService {
  constructor(
  ...
  async execute(data: IFilterStencilWashesDTO): Promise<any> {
    if (
    ...
    const { stencilWashes, ...rest } =
      await this.stencilWashRepository.findAllStencilWashesPaginate(data);
    ...
    return { stencilWashes: stencilWashesParse, ...rest };
  }
}
